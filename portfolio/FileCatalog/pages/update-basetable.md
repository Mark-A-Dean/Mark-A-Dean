...
The cmdlet uses the parameter values to establish a SQL Server connection to a database that hosts the named procedure. It then executes the named procedure, which requires the datatable object as its own User-Defined Table Type parameter.
...
|Type|String|
|Position|1|
|Default value|Provided by Get-FileDetails variable.|
|Required|True|
...
|Type|String|
|Position|3|
|Default value|Provided by Get-FileDetails string value.|
|Required|True|
